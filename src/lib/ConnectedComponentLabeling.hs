...
asssignLabelsM con arr = do
    ...
    let
        ...
        f ix acc e = tryToLabelNext con marr (e, ix) acc
      
    _ <- ifoldlMutM f (incrementLabel defaultLabel) marr
    unsafeFreeze (getComp arr) marr


tryToLabelNext :: forall m . (Mutable U Ix2 PixelL, PrimMonad m, MonadThrow m) => Connectivity -> MArray (PrimState m) U Ix2 PixelL -> PixelLIx -> Label -> m Label
tryToLabelNext a b c d = tryToLabel False a b c d


tryToLabelNeighbor :: forall m . (Mutable U Ix2 PixelL, PrimMonad m, MonadThrow m) => Connectivity -> MArray (PrimState m) U Ix2 PixelL -> PixelLIx -> Label -> m Label
tryToLabelNeighbor a b c d = tryToLabel True a b c d


tryToLabel :: forall m . (Mutable U Ix2 PixelL, PrimMonad m, MonadThrow m) => Bool -> Connectivity -> MArray (PrimState m) U Ix2 PixelL -> PixelLIx -> Label -> m Label
tryToLabel isNeighbor con marr (e, ix) l = do 
    let
...
handleNeighbors con marr ix l = do   
    let
        ...
        f acc offset = do
            ...
            case mbE of
                Just e -> do
                    _ <- tryToLabelNeighbor con marr (e, ix') l
                    acc
                Nothing -> acc
